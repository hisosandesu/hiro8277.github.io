...
### 7. AAB アップロード後「最新版になっていない」問題の調査・解決

**症状**: クローズドテストに AAB をアップロードしたが、Play Console 上で「最新版」として表示されなかった。

**原因**: AAB アップロード直後は Play Console の表示反映に時間差がある。  
「審査中」ステータスのリリースは「公開済み最新版」とは別扱いのため、ダッシュボード表示が遅延する。これは正常動作。

**解決**: しばらく待ってからインストールしたところ最新版に更新されたことを確認。アップロード・審査提出自体は正しく完了していた。

**教訓**: Play Console へのアップロード後、ダッシュボードへの反映は数分〜十数分かかる場合がある。エラーメッセージがなければ再アップロード不要。
